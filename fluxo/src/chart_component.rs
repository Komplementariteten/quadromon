...
use crate::text_render::{Alignment, render_text_normal, render_text_small_color};
...
pub fn render_chart(
    ...
    add_title(
        scene,
        title,
        offset as f64 + (2. * DEFAULT_MARGIN) + DEFAULT_STROKE_WIDTH,
    );
...
fn add_title(scene: &mut Scene, title: &str, offset: f64) {
    render_text_normal(
        scene,
        title,
        (2. * DEFAULT_MARGIN) + DEFAULT_STROKE_WIDTH,
        offset,
    );
}
...
fn add_plot(
    ...
    for i in 0..bar_values.len() {
        let rect = Rect::new(
            left_margin + (i * BAR_WIDTH) as f64,
            bar_values[i as usize],
            left_margin + ((i + 1) * BAR_WIDTH) as f64,
            bar_y_pos as f64,
        );
